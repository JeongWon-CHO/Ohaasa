...
GitHub Actions (cron: UTC 일~금 22:00 = JST/KST 월~토 07:00)
  └─ backend (Node.js/TypeScript)
       ...
       ├─ 3. GPT 번역 (advice_ko 생성 — 재번역 방지 포함)
       ├─ 4. Supabase horoscopes upsert (advice + advice_ko)
       ├─ 5. user_devices 조회 (Phase 4-2)
       └─ 6. Expo Push API 발송 (Phase 4-2)
...
`date`, `zodiac_sign`, `zodiac_name`, `rank`, `advice`, `advice_ko`

- `advice`: 일본어 원문 (크롤러 파싱 결과)
- `advice_ko`: GPT 번역된 한국어 조언 (nullable — 실패 시 NULL, 앱은 advice로 fallback)
...
**장기 방향**:
- 평일: 오하아사(おはよう朝日です) 별자리 운세 데이터 사용
- 주말: 고고별자리(グッド!モーニング 기반) 별자리 운세 데이터 사용 — 추후 별도 Phase에서 구현

**Step 3 MVP 범위**:
- 오하아사 Supabase 연동만 구현한다. 고고별자리 연동은 하지 않는다.
- 주말 fallback 로직(최신 오하아사 데이터를 "오늘 운세"처럼 표시)도 구현하지 않는다.
- DatePill은 "오늘"이 아닌 오하아사 방송 기준일(DB의 `date` 컬럼)을 명시한다.
- 방송 데이터가 없는 날(일요일 등)에는 데이터 없음 상태를 표시한다.
- 오하아사 API에 없는 필드(행운 아이템·색상·숫자, 연애·직장·금운·건강 별점)는 UI에서 주석 처리해 숨긴다. 추후 고고별자리 연동 Phase에서 복구 예정.
...
| `OPENAI_API_KEY`            | OpenAI API 키. **backend/GitHub Actions Secrets 전용. 앱 절대 노출 금지** |
| `OPENAI_MODEL`              | 번역 모델명. 기본값: `gpt-4o-mini` (GitHub Actions에서 환경변수로 주입) |
...
| Phase 6   | React Native Expo 앱                       | 🔄 진행 중 (Step 2 완료, Step 3 진행 중)                   |
...
| Step 3 | Supabase 연동: 오하아사 운세 조회 · 방송 기준일 표시 · 전체 순위 · 로딩/에러/데이터 없음 상태 (행운 아이템·별점 주석 처리) | 🔄 진행 중 |
